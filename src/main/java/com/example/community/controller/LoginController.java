...
public class LoginController implements CommunityConstant {
    ...
    @Autowired
    private Producer kaptchaPriducer;
...
//    http://local:8080/community/activation/101/code
    ...
    public String activation(Model model, @PathVariable("userId") int userId, @PathVariable("code") String code){
        ...
        if (result == ACTIVATION_SUCCESS){
            ...
            model.addAttribute("msg", "无效操作，该账号已完成激活！");
            model.addAttribute("target", "/index");
        }else{
...
//        生成验证码
        String text = kaptchaPriducer.createText();
        BufferedImage image = kaptchaPriducer.createImage(text);

//        将验证码存入session
//        session.setAttribute("kaptcha", text);

//        将验证码存入redis
//        验证码的归属者，生成一个随机字符串用于临时标识用户并存入cookie
